...
import ImageVerificationPage from './ImageVerificationPage';
...
// Minimal in-file translations hook to avoid missing module error.
// Replace or extract to ../hooks/useTranslations when you add a proper translations module.
...
const VerificationPage: React.FC<VerificationPageProps> = ({ language }) => {
  ...
  return (
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      {/* Tabs Container - Clean, no background */}
        ...
        <div className="flex flex-wrap justify-center gap-4 w-full max-w-4xl">
          {tabs.map((tab) => {
            ...
            return (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id as 'csv' | 'json' | 'image' | 'map')}
                className={`
      ...
      <div className="relative mt-6 min-h-[500px]">
        {/* 
          Using display: none (via hidden class) instead of conditional rendering 
          to preserve state (uploaded files, results) when switching tabs.
        */}
