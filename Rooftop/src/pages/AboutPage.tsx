...
import { Target, Zap, ShieldCheck, Sun, Home, LayoutGrid, IndianRupee, CheckCircle2, FileText, Settings, ArrowRight } from 'lucide-react';
    ...
    <div className="max-w-5xl mx-auto text-slate-800 dark:text-slate-200 space-y-12 pb-12">
      ...
      <div className="flex items-center justify-between w-full max-w-lg mx-auto relative px-4 sm:px-8 py-4">
        ...
        <div className="flex flex-col items-center z-10 relative group">
          <Sun size={48} className="text-yellow-400 sun-icon group-hover:scale-110 transition-transform" style={{ filter: 'drop-shadow(0 0 8px #facc15)' }} />
          <span className="mt-2 text-xs font-bold uppercase text-yellow-600 dark:text-yellow-400 tracking-wider">Sun</span>
        </div>

        {/* UV Rays: Sun -> Panel */}
        <div className="flex-grow mx-3 relative h-8 flex flex-col justify-center">
           <div className="uv-ray-path" style={{ top: '30%' }}></div>
           <div className="uv-ray-path" style={{ top: '50%', animationDelay: '-0.5s' }}></div>
           <div className="uv-ray-path" style={{ top: '70%', animationDelay: '-0.2s' }}></div>
        </div>
        ...
        <div className="flex flex-col items-center z-10 relative group">
           <div className="transform perspective-500 rotate-x-12 bg-slate-800 p-2 rounded border border-sky-500/30 shadow-lg group-hover:border-sky-400 transition-colors">
              <LayoutGrid size={32} className="text-sky-400 panel-icon" strokeWidth={1.5} />
           </div>
           <span className="mt-2 text-xs font-bold uppercase text-sky-600 dark:text-sky-400 tracking-wider">Solar Panel</span>
        </div>
        ...
        <div className="flex flex-col items-center z-10 relative group">
          <div className="relative">
            <Home size={48} className="text-slate-600 dark:text-slate-300 group-hover:text-slate-500 dark:group-hover:text-white transition-colors" />
            <Zap size={20} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-yellow-400 animate-pulse" fill="currentColor" />
          </div>
          <span className="mt-2 text-xs font-bold uppercase text-slate-600 dark:text-slate-300 tracking-wider">Home</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="glass-pane p-6 rounded-2xl border-l-4 border-sky-500 dark:border-sky-400 hover:shadow-lg transition-shadow">
            <h3 className="text-xl font-bold mb-4 flex items-center gap-2 text-slate-800 dark:text-slate-100">
                <CheckCircle2 className="text-sky-500" /> {t('about.eligibility.title')}
            </h3>
            <ul className="space-y-3">
                {[1, 2, 3, 4].map((item) => (
                    <li key={item} className="flex items-start gap-3 text-slate-600 dark:text-slate-300">
                        ...
                        <span>{t(`about.eligibility.item${item}` as any)}</span>
                    </li>
                ))}
            </ul>
          </div>
          ...
          <div className="glass-pane p-6 rounded-2xl border-l-4 border-green-500 dark:border-green-400 hover:shadow-lg transition-shadow">
            <h3 className="text-xl font-bold mb-4 flex items-center gap-2 text-slate-800 dark:text-slate-100">
                <IndianRupee className="text-green-500" /> {t('about.subsidy.title')}
            </h3>
            <div className="overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700">
                <table className="w-full text-left text-sm">
                    <thead className="bg-slate-100 dark:bg-slate-800">
                        <tr>
                            <th className="p-3 font-bold text-slate-700 dark:text-slate-300">Capacity</th>
                            <th className="p-3 font-bold text-slate-700 dark:text-slate-300">Subsidy Amount</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                        <tr className="bg-white/50 dark:bg-slate-900/50">
                            <td className="p-3 text-slate-600 dark:text-slate-300">1 kW</td>
                            <td className="p-3 font-bold text-green-600 dark:text-green-400">₹ 30,000</td>
                        </tr>
                        <tr className="bg-white/50 dark:bg-slate-900/50">
                            <td className="p-3 text-slate-600 dark:text-slate-300">2 kW</td>
                            <td className="p-3 font-bold text-green-600 dark:text-green-400">₹ 60,000</td>
                        </tr>
                        <tr className="bg-white/50 dark:bg-slate-900/50">
                            <td className="p-3 text-slate-600 dark:text-slate-300">3 kW +</td>
                            <td className="p-3 font-bold text-green-600 dark:text-green-400">₹ 78,000</td>
                        </tr>
                    </tbody>
                </table>
            </div>
      ...
      <div className="glass-pane p-8 rounded-2xl relative overflow-hidden">
          ...
          <div className="flex flex-wrap justify-center gap-8 relative z-10">
             <ProcessStep number="1" icon={<FileText size={20}/>} label={t('about.step.1')} />
             <ArrowRight className="hidden md:block text-slate-400 mt-4" />
             <ProcessStep number="2" icon={<Settings size={20}/>} label={t('about.step.2')} />
             <ArrowRight className="hidden md:block text-slate-400 mt-4" />
             <ProcessStep number="3" icon={<Zap size={20}/>} label={t('about.step.3')} />
             <ArrowRight className="hidden md:block text-slate-400 mt-4" />
             <ProcessStep number="4" icon={<ShieldCheck size={20}/>} label={t('about.step.4')} />
             <ArrowRight className="hidden md:block text-slate-400 mt-4" />
             <ProcessStep number="5" icon={<IndianRupee size={20}/>} label={t('about.step.5')} isLast />
          </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
            <BenefitCard title={t('about.benefit.financial.title')} description={t('about.benefit.financial.desc')} />
            <BenefitCard title={t('about.benefit.clean.title')} description={t('about.benefit.clean.desc')} />
            <BenefitCard title={t('about.benefit.independence.title')} description={t('about.benefit.independence.desc')} />
            <BenefitCard title={t('about.benefit.economic.title')} description={t('about.benefit.economic.desc')} />
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
            <BenefitCard title={t('about.benefit.transparency.title')} description={t('about.benefit.transparency.desc')} />
            <BenefitCard title={t('about.benefit.efficiency.title')} description={t('about.benefit.efficiency.desc')} />
            <BenefitCard title={t('about.benefit.insights.title')} description={t('about.benefit.insights.desc')} />
            <BenefitCard title={t('about.benefit.trust.title')} description={t('about.benefit.trust.desc')} />
          </div>
        </div>
      </div>
    </div>
...
const ProcessStep: React.FC<{ number: string; icon: React.ReactNode; label: string; isLast?: boolean }> = ({ number, icon, label, isLast }) => (
    <div className="flex flex-col items-center gap-3 group">
        <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white shadow-lg transition-all duration-300 group-hover:scale-110 ${isLast ? 'bg-green-500 shadow-green-500/30' : 'bg-sky-500 shadow-sky-500/30'}`}>
            {icon}
        </div>
        <div className="text-center">
            <div className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Step {number}</div>
...
const BenefitCard: React.FC<{ title: string; description: string }> = ({ title, description }) => (
    <div className="bg-white/40 p-4 rounded-lg border border-white/30 hover:bg-white/60 dark:bg-slate-800/40 dark:hover:bg-slate-700/60 dark:border-slate-700/50 transition-colors">
        <h4 className="font-bold text-sm text-slate-800 dark:text-slate-100 flex items-center gap-2 mb-1">
            <ShieldCheck size={16} className="text-green-600 dark:text-green-400" /> {title}
        </h4>
        <p className="text-xs text-slate-600 dark:text-slate-400 leading-snug">{description}</p>
    </div>
);
